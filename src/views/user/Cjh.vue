  ...
  <div>
    <el-table height="520px"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button style="margin-left: 30px" size="mini" type="text"
...
<script>export default {
  ...
  methods: {
    handleSizeChange(val) {
      const that = this
      this.axios.get('/user/adminListCjh?size=' + val).then(function (rest) {
    ...
    handleCurrentChange(val) {
      const that = this
      this.axios.get('/user/adminListCjh?size=' + that.page.size + '&current=' + val).then(function (rest) {
        that.page = rest.data.data
      }, function (error) {
        console.log(error)
      })
  ...
  created() {
    const that = this
    this.axios.get('/user/adminListCjh').then(function (rest) {
      that.page = rest.data.data
    }, function (error) {
      console.log(error)
    })
