...
<script>export default {
  ...
  methods: {
    ...
    handleCurrentChange (val) {
      const that = this
      this.axios.get('/forward/list?size=' + that.page.size + '&current=' + val).then(function (rest) {
        that.page = rest.data.data
      }, function (error) {
        console.log(error)
      })
    ...
    handleSizeChange2 (val) {
      const that = this
      this.axios.get('/forward/listAdmin?size=' + val).then(function (rest) {
    ...
    handleCurrentChange2 (val) {
      const that = this
      this.axios.get('/forward/listAdmin?size=' + that.page2.size + '&current=' + val).then(function (rest) {
        that.page2 = rest.data.data
      }, function (error) {
        console.log(error)
      })
  ...
  created() {
    ...
    this.axios.get('/forward/listAdmin').then(function (rest) {
      that.page2 = rest.data.data
    }, function (error) {
      console.log(error)
    })
