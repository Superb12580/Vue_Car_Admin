            ...
            <div class="ytxw" v-for="video in page.records">
                ...
                <div class="xwt">
                    <router-link :to="{path: '/spxq',query: {id: video.id}}">
                        <img style="width: 100%;height: 100%" v-if="video.videoUrl" :src="video.videoUrl">
                    </router-link>
                </div>
                <!--新闻右侧-->
                <div class="xwyc">
                    <router-link :to="{path: '/spxq',query: {id: video.id}}"><h3 style="height: 60px">{{video.videoTitle}}</h3>
                    </router-link>
                    <a href="#" @click="xq(video.user.userId)"><h4 style="float: left">{{video.user.userName}}</h4></a>
                    ...
                    <h5 style="font-size: 14px;float: right"><span style="font-size: 25px;margin-right: 6px"
                                                                   class="el-icon-s-help"></span>在看：{{video.count}}</h5>
                </div>
                <div style="float: right;margin-top: 60px">
                    <el-button type="text" @click="bh(video)"><img style="width: 60px" src="../../assets/驳回.png">驳回</el-button>
                    <el-button type="text" @click="fb(video)"><img style="width: 60px" src="../../assets/通过.png">通过</el-button>
                </div>
            </div>
...
<script>export default {
    ...
    methods: {
        handleSizeChange(val) {
            const that = this
            this.axios.get('/video/listAdminDsh?size=' + val).then(function (rest) {
        ...
        handleCurrentChange(val) {
            const that = this
            this.axios.get('/video/listAdminDsh?size=' + that.page.size + '&current=' + val).then(function (rest) {
                that.page = rest.data.data
            }, function (error) {
                console.log(error)
            })
    ...
    created() {
        const that = this
        // 所有
        this.axios.get('/video/listAdminDsh').then(function (rest) {
            that.page = rest.data.data
        }, function (error) {
            console.log(error)
        })
