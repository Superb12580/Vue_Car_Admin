  ...
  <div>
    <div v-if="page.total !== 0" class="left2">
      ...
      <div class="ytxw" v-for="video in page.records">
        ...
        <!--新闻右侧-->
        <div class="xwyc">
          <router-link :to="{path: '/spxq',query: {id: video.id}}"><h3 style="height: 60px">{{video.videoTitle}}</h3></router-link>
...
<script>export default {
  ...
  methods: {
    handleSizeChange (val) {
      const that = this
      this.axios.get('/video/listAdminYbh?size=' + val).then(function (rest) {
    ...
    handleCurrentChange (val) {
      const that = this
      this.axios.get('/video/listAdminYbh?size=' + that.page.size + '&current=' + val).then(function (rest) {
        that.page = rest.data.data
      }, function (error) {
        console.log(error)
      })
    ...
    fb(video) {
      ...
      this.$confirm('此操作将变更设置, 是否继续?', '提示', {
        ...
        this.axios.post('/video/fbAdmin', { id: video.id, userId: video.user.userId, videoTitle: video.videoTitle}).then(function (rest) {
  ...
  created () {
    const that = this
    // video
    this.axios.get('/video/listAdminYbh').then(function (rest) {
      that.page = rest.data.data
    }, function (error) {
      console.log(error)
    })
