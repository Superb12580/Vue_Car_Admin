...
<script>export default {
  ...
  methods: {
    handleSizeChange (val) {
      const that = this
      this.axios.get('/video/list?size=' + val).then(function (rest) {
    ...
    handleCurrentChange (val) {
      const that = this
      this.axios.get('/video/list?size=' + that.page.size + '&current=' + val).then(function (rest) {
        that.page = rest.data.data
      }, function (error) {
        console.log(error)
      })
    ...
    bh(video) {
      ...
      this.$confirm('此操作将变更设置, 是否继续?', '提示', {
        ...
        this.axios.post('/video/bhAdmin', { id: video.id, userId: video.user.userId, videoTitle: video.videoTitle}).then(function (rest) {
  ...
  created () {
    const that = this
    this.axios.get('/video/list').then(function (rest) {
      that.page = rest.data.data
    }, function (error) {
      console.log(error)
    })
