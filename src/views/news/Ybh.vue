...
<script>export default {
  ...
  methods: {
    handleSizeChange (val) {
      const that = this
      this.axios.get('/news/listAdminYbh?size=' + val).then(function (rest) {
    ...
    handleCurrentChange (val) {
      const that = this
      this.axios.get('/news/listAdminYbh?size=' + that.page.size + '&current=' + val).then(function (rest) {
        that.page = rest.data.data
      }, function (error) {
        console.log(error)
      })
    ...
    fb(news) {
      ...
      this.$confirm('此操作将变更设置, 是否继续?', '提示', {
        ...
        this.axios.post('/news/fbAdmin', { id: news.id, userId: news.user.userId, title: news.title}).then(function (rest) {
  ...
  created () {
    const that = this
    // 所有news
    this.axios.get('/news/listAdminYbh').then(function (rest) {
      that.page = rest.data.data
    }, function (error) {
      console.log(error)
    })
