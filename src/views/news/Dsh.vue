    ...
    <div>
        <div v-if="page.total !== 0" class="left2">
            ...
            <div class="ytxw" v-for="news in page.records">
                ...
                <!--新闻右侧-->
                <div class="xwyc">
                    <router-link :to="{path: '/wzxq',query: {id: news.id}}"><h3 style="height: 60px">{{news.title}}</h3>
                    </router-link>
                    ...
                    <h5 style="font-size: 14px;float: right"><span style="font-size: 25px;margin-right: 6px"
                                                                   class="el-icon-s-help"></span>在看：{{news.count}}</h5>
                </div>
                <div style="float: right;margin-top: 60px">
                    <el-button type="text" @click="bh(news)"><img style="width: 60px" src="../../assets/驳回.png">驳回</el-button>
                    <el-button type="text" @click="fb(news)"><img style="width: 60px" src="../../assets/通过.png">通过</el-button>
                </div>
            </div>
...
<script>export default {
    ...
    methods: {
        handleSizeChange(val) {
            const that = this
            this.axios.get('/news/listAdminDsh?size=' + val).then(function (rest) {
        ...
        handleCurrentChange(val) {
            const that = this
            this.axios.get('/news/listAdminDsh?size=' + that.page.size + '&current=' + val).then(function (rest) {
                that.page = rest.data.data
            }, function (error) {
                console.log(error)
            })
    ...
    created() {
        const that = this
        // 所有news
        this.axios.get('/news/listAdminDsh').then(function (rest) {
            that.page = rest.data.data
        }, function (error) {
            console.log(error)
        })
